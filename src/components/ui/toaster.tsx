...
export function Toaster() {
  ...
  return (
    <ToastProvider>
      <div className="fixed bottom-0 right-0 z-[100] flex flex-col-reverse p-4 sm:bottom-0 sm:right-0 sm:top-auto sm:flex-col-reverse md:max-w-[420px]">
        {toasts.map(function ({ id, title, description, action, variant, className, ...props }, index) {
          ...
          // Add staggered entry delay based on index
          ...
          return (
            <SlotMachineToast key={id} style={{ transitionDelay: `${entryDelay}ms` }}>
              <Toast 
                {...props} 
                className={cn(
                  "animate-enter flex items-start gap-3 border-l-4 my-2 shadow-lg", 

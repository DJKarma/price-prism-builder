...
export const exportConfig = (config: PricingConfig): string => {
...
export const importConfig = async (file: File): Promise<{ config: PricingConfig, unmatchedFields: string[] }> => {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const unmatchedFields = Object.keys(importedConfig).filter(key => !knownFields.has(key));
        
        resolve({
          config: importedConfig as PricingConfig,
          unmatchedFields
        });
      } catch (error) {

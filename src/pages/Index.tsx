...
const Index = () => {
  ...
  const handleMappingComplete = (mapping: Record<string, string>, rows: any[], categories: {
    column: string;
    categories: string[];
  }[]) => {
    ...
    const defaultConfig = {
      basePsf: BASE_PSF,
      bedroomTypePricing: uniqueTypes.map(t => ({
        type: t,
        basePsf: BASE_PSF,
        targetAvgPsf: BASE_PSF
      })),
      viewPricing: uniqueViews.map(v => ({
        view: v,
        psfAdjustment: 0
      })),
      floorRiseRules: [{
      ...
      additionalCategoryPricing: categories.flatMap(cat => cat.categories.map(val => ({
        column: cat.column,
        category: val,
        psfAdjustment: 0
      }))),
  ...
  const handleConfigUpdate = useCallback((cfg: any) => {
  ...
  const isTabDisabled = (t: string) => t === "map" && !csvData.length || t === "simulate" && !mappedData.length;
  ...
  return <div className="min-h-screen bg-gray-50">
      ...
      {/* -------- main -------- */}
      <main className="container mx-auto px-4 py-8 font-thin">
        <Tabs value={activeTab} onValueChange={handleManualTab}>
          ...
          <TabsList className="grid grid-cols-3 w-full bg-gray-100 p-1 rounded-lg shadow-inner mb-8">
            ...
            const past = step === "upload" && ["map", "simulate"].includes(activeTab) || step === "map" && activeTab === "simulate";
            return <TabsTrigger key={step} value={step} disabled={isTabDisabled(step)} className={`
                  ...
                  {i < 2 && <div className="absolute -right-3 top-1/2 h-0.5 w-6 bg-gray-300 -translate-y-1/2 hidden md:block" />}
                </TabsTrigger>;
          })}
          </TabsList>
          ...
          <div className={`transition-all duration-300 ${tabChanging ? transitionDirection === "forward" ? "opacity-0 translate-x-10" : "opacity-0 -translate-x-10" : "opacity-100 translate-x-0"}`}>
            ...
            <TabsContent value="map">
              {csvData.length && csvHeaders.length ? <ColumnMapper headers={csvHeaders} data={csvData} onMappingComplete={handleMappingComplete} onBack={() => changeTab("upload", "backward")} /> : null}
            </TabsContent>
            ...
            <TabsContent value="simulate">
              {mappedData.length ? <>
                  <div className="mb-4">
                    <Button variant="outline" size="sm" onClick={() => changeTab("map", "backward")}>
                  ...
                  <MegaOptimize data={mappedData} pricingConfig={pricingConfig} onOptimized={handleConfigUpdate} />

                  {/* pricing config + table */}
                  <PricingSimulator data={mappedData} pricingConfig={pricingConfig} onConfigUpdate={handleConfigUpdate} additionalCategories={additionalCategories} maxFloor={maxFloor} key={`sim-${forceUpdate}`} />
                </> : null}
            </TabsContent>
          </div>
        </Tabs>
